...
async function approve(){
    ...
    let contract = new ethers.Contract("0xDE469cBECF11329fBc0a87f1011b62353876b5F4", approveabi, walletWithProvider);
    try {
		console.log(11111);
        var approve = await contract.approve("0xB34a35eD11Db9DeC8f79c3Ff3F031C8fe19F7121","100000000000000000000000000");
        await approve.wait(); 
    } catch (error) {
        loadingStop();
...
async function rechagreHandle(amount) {
    ...
    loadingStart();
    let Ucontract = new ethers.Contract("0xDE469cBECF11329fBc0a87f1011b62353876b5F4", abi1, walletWithProvider);
     ...
     if(two<0){
        loadingStop();
        Dreamer.error("Your Token is not enough",2000);
         return;
     }
	 var abi = [
		{
    ...
    let contract = new ethers.Contract("0xB34a35eD11Db9DeC8f79c3Ff3F031C8fe19F7121", abi, walletWithProvider);
    try {
       
            var claim = await contract.rechagreHandle(amountNew);
            await claim.wait()
	...
	try {
	  ...
	  loadingStop();
	  Dreamer.error("rechagre1 fail",2000);
		 return
	}
	   console.log(datas)
	...
	var kaimi2 = datas?datas.data.data:"";

    loadingStop();
    Dreamer.success("rechagre is success",2000);
    return kaimi2;
}

async function withdrawHandle(amount,kamiCan) {
	...
	let dataTest = {  
		coins:amount,
	cdKey:kamiCan};
	var datas =  await  axios.post(urlTest,dataTest);
	var code = datas?datas.data.code:0;
	   if(code != 200){
		  loadingStop();
		  Dreamer.error("卡密不存在",2000);
		 return
	   }
	} catch (error) {
		loadingStop();
    ...
    try {
            ...
            var claim = await contract.withdrawApply(amountNew);
            await claim.wait()
	...
	try {
	  ...
	  loadingStop();
	  Dreamer.error("withdraw fail",2000);
		 return
	}
	   console.log(datas)
	   var code = datas?datas.data.code:0;
    ...
    loadingStop();
    Dreamer.success("withdraw is success",2000);
    return;
}
...
async function idoHandle(amount){
    loadingStart();
    var idoAbi=[
        {
            "inputs": [],
...
async function claimHandle(){
    ...
    try {
        ...
        loadingStop();
        Dreamer.error("claim token fail",2000);
        return;
    }
  
    loadingStop();
    Dreamer.success("claim token",2000);
    return true;
}
